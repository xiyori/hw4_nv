...
parser = argparse.ArgumentParser(description="Train HiFi-GAN model.")
...
mel_spectrogram_loss = mel_config.mel_spectrogram_loss.to(train_config.device)
...
for _ in range(args.n_tries):
    reply = train(train_config,
                  mel_spectrogram, mel_spectrogram_loss,
                  train_loader, valid_loader,
                  generator, discriminator,
                  gen_optimizer, dis_optimizer,
                  gen_scheduler, dis_scheduler,
                  resume, args.progress)
    if reply == "success":
        exit()
